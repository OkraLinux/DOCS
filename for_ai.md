...
- 新建的跨动态库、插件或跨语言导出遵守 `oaabi.md`。
- 导出使用 C 链接和 PascalCase。签名里只有 C 的整数、浮点、指针和不完全结构体指针。
- 聚合类型用指针传递。失败返回负 errno。
- 禁止在导出签名里放置 C++ 类、Rust 对象、`PyObject`、`FILE` 或 `long double`。
- 新插件导出 `struct OaabiPlugin`。不要改已有的 `lunar_plugin_init`。
